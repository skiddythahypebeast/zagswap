...
export const InputLabel = ({ label }: { label: { default: string, error: [string, string] | undefined } }) => {
    return (
        <div className={`w-full bg-bg4 dark:bg-dark-bg4 rounded-lg py-1 flex flex-row justify-between relative`}>
            {!label.error && <p className="text-text dark:text-dark-text font-bold px-2 text-sm absolute">{label.default}</p>}
            {label.error && (
