...
export const OrderDetails = ({ order_details, currency_from, currency_to, order_id }: { order_details: GetOrderResponse, currency_from: GetCurrencyResponse, currency_to: GetCurrencyResponse, order_id: string }) => {
    ...
    return (
        <div className="w-[500px] xl:max-w-[80%] lg:max-w-[80%] md:max-w-[80%] max-w-[95%] gap-2 flex flex-col bg-three dark:bg-three-dark dark:shadow-shade-dark rounded-xl p-5">
            <CopyButton value={orderDetails.id} className="dark:border-border-dark dark:border-[1px] px-2 py-1 rounded-md lex justify-between w-full flex flex-row gap-2 items-center opacity-70 hover:opacity-100">
                ...
                <Image src="/icons/copy.svg" className="dark:filter dark:invert" height={10} width={10} alt="Icon"/>
            </CopyButton>
            ...
            {currency_from.has_extra_id && orderDetails.extra_id_from && statusIndex < 4 && <CopyButton 
                ...
                className="flex flex-row rounded-lg items-center justify-between px-5 bg-amber-500/30 border-b-2 border-b-amber-500 py-2">
        ...
        <div className="flex flex-col gap-2 items-center rounded-lg p-5">
            <div className="flex flex-col items-center gap-2">
                ...
                <div className="h-40 flex items-center justify-center">
                    ...
                    {status === "expired" && <Image src="/icons/expired.svg" className="dark:filter dark:invert opacity-20" height={150} width={150} alt="Icon"/>}
                </div>
                ...
                {status === "finished" && <div className="flex flex-col gap-2 items-center">
                    ...
                    <p className="text-2xl font-bold text-primary dark:text-primary-dark">{orderDetails.amount_to} {orderDetails.currency_to.toUpperCase()}</p>
                </div>}
            </div>
            {!showDetails && <div className="flex flex-row gap-2">
                <button onClick={() => toggleDetails(true)} className="px-5 py-2 border-primary dark:border-primary-dark border-2 bg-primary/20 dark:bg-primary-dark/20 w-fit rounded-lg">
                    <p className="text-primary dark:text-primary-dark">order details</p>
                </button>
                <Link 
...
const DepositDetails = ({ from, amount, currency }: { from: string, amount: string, currency: GetCurrencyResponse }) => {
    return (
        <div className="p-2 gap-2 flex flex-col dark:border-primary-dark border-primary border-2 rounded-lg">
...
const RecieveDetails = ({ amount, from, title, currency, direction, extraId }: { currency: GetCurrencyResponse, extraId: string | null, amount: string, from: string, title: string, direction: "to" | "from" }) => {
    return (
        <div className="flex flex-col gap-2 mt-5">
            <div className="bg-four dark:bg-four-dark px-2 py-1 rounded-md justify-between flex flex-row gap-2">
                <p className="text-sm font-bold">{title}</p>
            </div>
            <CopyButton value={from} className="group flex flex-row justify-between h-12 gap-5 items-center xl:px-5 lg:px-5 md:px-5 px-3 bg-one dark:bg-input-dark shadow-sm shadow-shade dark:shadow-shade-dark rounded-lg">
            ...
            <div className="flex flex-row h-10 py-1 xl:px-5 lg:px-5 md:px-5 px-3 items-center rounded-md justify-between bg-one dark:bg-input-dark shadow-sm shadow-shade dark:shadow-shade-dark">
                ...
                <div className="flex flex-row gap-2">
                    ...
                    <div className="fade-in px-2 rounded-full flex items-center justify-center" style={{ backgroundColor: CHAIN_COLORS[currency.network] }}>
                        <p className="text-xs font-bold">{currency.network.toUpperCase()}</p>
                    </div>
                </div> 
            </div>
            {currency.has_extra_id && !!extraId && <CopyButton value={extraId} className="group flex flex-row justify-between h-10 gap-5 items-center xl:px-5 lg:px-5 md:px-5 px-3 bg-one dark:bg-input-dark shadow-sm shadow-shade dark:shadow-shade-dark rounded-lg">
                <div className="flex flex-row h-8 gap-2 items-center w-1/2 rounded-md py-1">
...
const OrderProgress = ({ status }: { status: OrderStatus }) => {
    ...
    return (
        <div className="flex flex-col gap-2">
            <div className="bg-three dark:bg-three-dark px-2 py-1 rounded-md justify-between flex flex-row gap-2">
                <p className="text-sm font-bold">Progress</p>
            </div>
            <div className="flex flex-row w-full gap-2 px-5">
                <div className="flex-1">
                    <div className={`${statusIndex == 0 ? "bg-primary dark:bg-primary-dark animate-pulse" : statusIndex > 0 ? "bg-primary dark:bg-primary-dark" : "bg-five dark:bg-input-dark"} h-3 rounded-full`}/>
                    <p className="text-xs text-center">pending deposit</p>
                </div>
                <div className="flex-1">
                    <div className={`${statusIndex == 1 ? "bg-primary dark:bg-primary-dark animate-pulse" : statusIndex > 1 ? "bg-primary dark:bg-primary-dark" : "bg-five dark:bg-input-dark"} h-3 rounded-full`}/>
                    <p className="text-xs text-center">funds received</p>
                </div>
                <div className="flex-1">
                    <div className={`${statusIndex == 2 ? "bg-primary dark:bg-primary-dark animate-pulse" : statusIndex > 2 ? "bg-primary dark:bg-primary-dark" : "bg-five dark:bg-input-dark"} h-3 rounded-full`}/>
                    <p className="text-xs text-center">exchanging</p>
                </div>
                <div className="flex-1">
                    <div className={`${statusIndex == 3 ? "bg-primary dark:bg-primary-dark animate-pulse" : statusIndex > 3 ? "bg-primary dark:bg-primary-dark" : "bg-five dark:bg-input-dark"} h-3 rounded-full`}/>
