        ...
        <div className="flex flex-col gap-2 w-full">
            ...
            <InputContainer position="center">
                <input 
                    ...
                    className="h-full w-full text-text dark:text-dark-text xl:text-lg lg:text-lg md:text-lg text-md outline-none py-2 px-5 bg-transparent rounded-md truncate" 
                />
            </InputContainer>
        </div>
    )
...
export const ExtraId = ({ currency, onChange, validator }: ExtraIdProps) => {
    ...
    return (
        <div className="flex flex-col gap-2 w-full">
            <div className={`w-full bg-bg4 dark:bg-dark-bg4 rounded-lg py-1 px-2 flex flex-row justify-between relative`}>
                <p className={`${!valid ? "opacity-100" : "opacity-0"} text-red-500 font-bold text-sm absolute`}>Invalid {currency.extra_id}</p>
                <p className={`${valid ? "opacity-100" : "opacity-0"} text-text dark:text-dark-text font-bold text-sm absolute`}>{currency.extra_id}</p>
                ...
                {valid && <Link 
                    ...
                    <img src="/icons/info.svg" className="w-5 h-5 dark:filter dark:invert" alt="Icon"/>
                </Link>}
            </div>
            <InputContainer position="center" size="small">
                <input 
                    ...
                    className={`w-full text-text dark:text-dark-text xl:text-md lg:text-md md:text-md text-md outline-none py-2 px-5 bg-transparent rounded-md truncate`} 
                />
            </InputContainer>
        </div>
    )
